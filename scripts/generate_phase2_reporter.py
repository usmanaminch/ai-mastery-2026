...
ROOT = Path(".").resolve()
PROMPT_PATH = ROOT / "prompts" / "phase2_verification_reporter.md"
...
Rules:
- Only write files under eval/report/ or tests/.
- Do not modify eval/patch_score/.
- Do not modify website files.
- Do not modify zlib artifacts.
...
def main() -> None:
    ...
    raw_path = ROOT / "phase2_reporter_raw_gemini_output.txt"
    ...
    if not files:
        raise SystemExit(
            ...
            "phase2_reporter_raw_gemini_output.txt"
        )
    ...
    print("Raw output saved to phase2_reporter_raw_gemini_output.txt")
